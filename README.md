...
See [QUICK_START.md § Validated toolchain versions](QUICK_START.md#validated-toolchain-versions-known-good-for-kserve-v0160) for sample passing/failing output, the full criticality breakdown (🔴 must match / 🟡 should match / 🟢 nice to match), and the `--customer-registry` mode that elevates skopeo to REQUIRED.
