...
> **Operator's home namespace is hardcoded to `kserve-operator-system`** in all generated artifacts. Customers customize at **deploy time** via env vars on the helper scripts (`OPERATOR_NAMESPACE` on `install-operator-deployment.sh`, `OPERATOR_NS` on `deploy-bundle.sh`, `SYSTEM_NS` on `setup-credentials.sh`). See [`extra-docs/architecture-namespaces.md` § 9](extra-docs/architecture-namespaces.md#9-design-c-footprint-always-2-namespaces-of-ours) for the full Design C model.
