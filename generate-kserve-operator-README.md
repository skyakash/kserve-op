...
- `--operator-namespace <ns>` : Operator's home namespace baked into all generated artifacts (default: `kserve-operator-system`). Substitutes the value into `operator-deployment.yaml` (Namespace + SA + RBAC + Service + Deployment), `setup-credentials.sh`'s `SYSTEM_NS` default, `deploy-bundle.sh`'s `OPERATOR_NS` default, and `install-operator-deployment.sh`'s `OPERATOR_NS_DEFAULT` placeholder. Optional — customers can also pick the operator namespace at deploy time via env vars on the helper scripts (no rebuild needed). Useful when an org wants a corporate naming convention (e.g. `kf-system`, `mlops-kserve-operator`) baked in once.
...
|---|---|---|
| `setup-credentials.sh` | Always | Creates pull secret in exactly **2 namespaces** (`default` + the operator's home, default `kserve-operator-system`) per Design C. Both deploy paths (OLM and direct-manifest) use the same 2-namespace footprint. Pre-flight checks cert-manager + the operator namespace; fails fast if missing. Accepts `--user`/`--pass` CLI args or prompts interactively. Runtime `SYSTEM_NS=<ns>` env var overrides the baked-in default. *(Deprecated `--non-olm` flag is silently accepted for backwards compat.)* |
