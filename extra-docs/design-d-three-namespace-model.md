...
Upstream KServe's release manifest (`kserve-source/install/v0.16.0/kserve.yaml:1-8`) ships a `Namespace` manifest carrying three labels:
...
- **Upstream Namespace manifest with offending label:** `kserve-source/install/v0.16.0/kserve.yaml:1-8` (verified pass-through to `p-kserve-raw/04-kserve-core/kserve-core.yaml:1-8`).
