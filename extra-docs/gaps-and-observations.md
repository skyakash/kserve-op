...
**⚠️ Version-specific caveat added 2026-07-11:** the "controller still honors the legacy alias" statement holds fully on `feat/kserve-0.16` (KServe v0.16.0) — an explicit `deploymentMode: "RawDeployment"` annotation still gets `scaleMetric` validated normally. On `feat/kserve-0.17` (KServe v0.17.0), upstream changed the defaulting webhook to also normalize *explicit* `RawDeployment` annotations to `"Standard"` before validation runs (not just absent ones, as on v0.16.0) — so the implicit scaleMetric safety net is lost on v0.17 even for charts that hardcode the legacy string. Not a breakage (charts still deploy fine either way), but v0.17's T31 negative case had to be redesigned to use `deploymentMode: "ModelMesh"` instead. Full detail on `feat/kserve-0.17`'s copy of `kserve-version-comparison.md` § "ConfigMap default flip side effect".
