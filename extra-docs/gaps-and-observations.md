...
**Fix:** Single-string flip in the Python ConfigMap rewrite block (`generate-kserve-raw.sh:345`) + sample template annotation + all downstream docs (README, generate-kserve-raw-README, kserve-raw-base/README.md.tmpl, QUICK_START Part C, package-readme.md.tmpl, air-gapped-examples.md ×2, architecture-overview mermaid, kserve-version-comparison "Effect on our project" paragraph, migration-journey mermaid). Regenerated `p-kserve-raw/` + `p-kserve-operator/` outputs to sync. **Landed:** commits `8051811`/`0234304`/`d310183` on `feat/kserve-0.16`, cherry-picked to `feat/kserve-0.17` as `9d40fd3`/`e43e662`/`95a2318`+`7a80aaa`. Verified by **T31** (cluster — scaleMetric webhook regression: negative case rejected, positive case with `Standard`+`hpa` annotations deploys clean) + **T32** (no-cluster — static grep confirms ConfigMap + sample both say `Standard`). **Downstream user impact:** none — controller still honors the legacy alias for any user chart hardcoding `"RawDeployment"`.

**Follow-up finding (2026-07-11, discovered during the v0.17 full test round):** the "controller still honors the legacy alias" statement above needs a version-specific caveat. On `feat/kserve-0.16` (KServe v0.16.0), an ISVC with an explicit `deploymentMode: "RawDeployment"` annotation still gets its `scaleMetric` validated (T31's negative case relies on this). On `feat/kserve-0.17` (KServe v0.17.0), upstream's defaulting webhook was changed to also normalize *explicit* `RawDeployment` annotations to `"Standard"` before validation runs — not just absent ones, as on v0.16.0. Practical effect: charts still deploy fine on both versions (no breakage), but the implicit scaleMetric safety net for explicit-legacy-annotation charts is lost on v0.17 specifically. v0.17's T31 negative case was redesigned to use `deploymentMode: "ModelMesh"` instead. Full source-level writeup, version-differentiated: `kserve-version-comparison.md` § "ConfigMap default flip side effect".
